       ...
       for(var key in json){
           if(key == 'goods'){
            ...
            var header = '<div class="header">'+
                        ...
                        '</div>'+
                        '<div class="list">'+
                        '<div class="commodity1"><a href="#"><img src="'+json[key][j][1].img+'" alt=""></a></div>'
            str += header
            for(var i=2;i<json[key][j].length;i++){
                var com = '<div class="com">'+
                '<a href="#">'+
                    '<img src="'+json[key][j][i].img+'" alt="">'+
                '</a>'+
                    '<h3>'+json[key][j][i].title+'</h3>'+
                    '<h5>'+
                        json[key][j][i].data+
                    '</h5>'+
                    '<div class="price">'+
                        '<span class="_1">'+json[key][j][i].price1 +'</span>'+
                        '<span class="_2">'+json[key][j][i].price2+'</span>'+
                    '</div>'+
            '</div>'
                str+=com
            }
...
// console.log(oa)
var nav_list = document.querySelector('.main_title .nav_list')
        ...
        success:function(json){
           ...
           if(e.target.innerHTML == "手机" || e.target.innerHTML == '抖音热销' || e.target.innerHTML == '抖音文创'){
               ...
               for(var i=0;i<json.nav_phone.length;i++){
                    str += '<a href="#">'+
                    '<img src="'+json.nav_phone[i].img+'" alt="">'+
                    '<p class="p1">'+json.nav_phone[i].title+'</p>'+
                   ' <p class="p2">'+json.nav_phone[i].price+'<span>起</span></p>'+
                '</a>'
               }
               $('.nav_list').append(str)
            ...
            for(var i=0;i<json.nav_clear.length;i++){
                 str += '<a href="#">'+
                 '<img src="'+json.nav_clear[i].img+'" alt="">'+
                 '<p class="p1">'+json.nav_clear[i].title+'</p>'+
                ' <p class="p2">'+json.nav_clear[i].price+'<span>起</span></p>'+
             '</a>'
            }
            $('.nav_list').append(str)
            ...
            for(var i=0;i<json.nav_pj.length;i++){
                 str += '<a href="#">'+
                 '<img src="'+json.nav_pj[i].img+'" alt="">'+
                 '<p class="p1">'+json.nav_pj[i].title+'</p>'+
                ' <p class="p2">'+json.nav_pj[i].price+'<span>起</span></p>'+
             '</a>'
            }
            $('.nav_list').append(str)
           }
